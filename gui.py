...
class PLCCodeGeneratorApp(tk.Tk):
    ...
    def _build_form(self, parent):
        ...
        # Variables
        self.cont_name_var = tk.StringVar()
        ...
        # Controller Name
        ttk.Label(form, text="Controller Name:").grid(row=0, column=0, padx=(0, 8), pady=6, sticky="e")
        ttk.Entry(form, textvariable=self.cont_name_var).grid(row=0, column=1, columnspan=2, sticky="ew", pady=6)

        # Conveyor Name
        ttk.Label(form, text="Conveyor Name:").grid(row=1, column=0, padx=(0, 8), pady=6, sticky="e")
        ttk.Entry(form, textvariable=self.conv_name_var).grid(row=1, column=1, columnspan=2, sticky="ew", pady=6)

        # Number of Conveyors
        ttk.Label(form, text="Number of Conveyors:").grid(row=2, column=0, padx=(0, 8), pady=6, sticky="e")
        ttk.Entry(form, textvariable=self.num_of_conv_var).grid(row=2, column=1, columnspan=2, sticky="ew", pady=6)
        ...
        # Filepath
        ttk.Label(form, text="Filepath to write code into:").grid(row=4, column=0, padx=(0, 8), pady=6, sticky="e")
        ttk.Entry(form, textvariable=self.filepath_var).grid(row=4, column=1, sticky="ew", pady=6)
        ttk.Button(form, text="📂 Browse...", command=self.browse_file).grid(row=4, column=2, padx=(8,0), pady=6, sticky="w")

    def browse_file(self):
        file_path = filedialog.asksaveasfilename(
            defaultextension=".L5K",
            filetypes=[("L5K Files", "*.L5K"), ("All files", "*.*")]
        )
        if file_path:
            self.filepath_var.set(file_path)

    def save_input(self):
        cont_name = self.cont_name_var.get().strip()
        conv_name = self.conv_name_var.get().strip()
        num_of_conv = self.num_of_conv_var.get().strip()
        is_MU = self.is_MU_var.get()
        filepath = self.filepath_var.get().strip()

        if not cont_name or not conv_name or not num_of_conv or not filepath:
        ...
        try:
            ...
            code = Conveyor.generate_plc_code_full(conv_name, num, is_MU, cont_name)
